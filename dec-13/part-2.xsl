...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="xsl:initial-template">
        ...
        <xsl:variable name="folded-sheet" as="map(*)"
            select="fold-left($folds, $initial-sheet, tdl:fold#2)"/>

        <xsl:variable name="max-x" as="xs:integer"
            select="max(map:keys($folded-sheet) ! $folded-sheet(.)?x)"/>
        <xsl:variable name="max-y" as="xs:integer"
            select="max(map:keys($folded-sheet) ! $folded-sheet(.)?x)"/>

        <xsl:for-each select="0 to $max-y">
            <xsl:variable name="y" as="xs:integer" select="."/>
            <xsl:for-each select="0 to $max-x">
                <xsl:variable name="x" as="xs:integer" select="."/>

                <xsl:value-of
                    select="if (exists($folded-sheet(tdl:point-key(map{'x':$x, 'y':$y}))))
                            then '#'
                            else '.'"/>
            </xsl:for-each>

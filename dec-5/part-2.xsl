...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="xsl:initial-template">
        <xsl:variable name="lines" as="map(*) +"
            select="($input ! tdl:parse-input(.) ! tdl:normalize-line(.))"/>
    ...
    <xsl:function name="tdl:points" as="map(*) +">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$line?y1 eq $line?y2">
            ...
            <xsl:otherwise>
                <xsl:variable name="slope" as="xs:integer"
                    select="if ($line?y2 gt $line?y1) then 1 else -1"/>

                <xsl:for-each select="0 to ($line?x2 - $line?x1)">
    ...
    <xsl:function name="tdl:normalize-line" as="map(*)">
        <xsl:param name="line" as="map(*)"/>

        <xsl:choose>
            <xsl:when test="$line?x1 gt $line?x2">
                <xsl:sequence select="
                    map {
                        'x1': $line?x2,
                        'y1': $line?y2,

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="xsl:initial-template">
        <xsl:iterate select="$bingo-numbers">
            ...
            <xsl:variable name="non-winning-boards"
                select="$modified-boards[not(tdl:has-win(.))]"/>

            <xsl:choose>
                <xsl:when test="count($non-winning-boards) eq 0">
                    <xsl:variable name="winning-board" select="$modified-boards[last()]"/>
...
Product:             {$unmarked-number-sum * $number}</xsl:break>
                ...
                <xsl:otherwise>
                    <xsl:next-iteration>
                        <xsl:with-param name="boards" select="$non-winning-boards"/>
                    </xsl:next-iteration>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:iterate>
